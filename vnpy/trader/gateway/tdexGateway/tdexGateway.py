...
from vnpy.api.tdex import TdexRestApi, TdexWebsocketApi
...
class RestApi(TdexRestApi):
    ...
    def sendOrder(self, orderReq):
        ...
        data = {
            'cid': 1,
            'side': 0 if orderReq.direction == '多' else 1,
            'scale': 20,
            'volume': orderReq.volume,
            'visible': -1,
        }

        if str(orderReq.priceType) == '限价':
            print('进入现价')
            data['price'] = orderReq.price

        print(data)
        res = tdex.futuresOpen(data)
        print(res)
...
########################################################################
class WebsocketApi(TdexWebsocketApi):
    """"""
